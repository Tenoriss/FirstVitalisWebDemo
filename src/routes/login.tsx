...
function LoginPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      const isAdmin = email.toLowerCase() === "admin@vitalis.com";
    ...
    <div className="container mx-auto flex min-h-[80vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-3xl border bg-card p-8 shadow-luxe animate-scale-in">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <Button type="submit" className="w-full bg-primary-gradient shadow-soft">Sign In</Button>
        </form>
        <p className="mt-6 text-center text-sm text-muted-foreground">
          Belum punya akun? <Link to="/register" className="font-semibold text-primary hover:underline">Register</Link>
        </p>
